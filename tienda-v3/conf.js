/* ============================================================
   ...
   Cambio de la v3: el ultimo paso ya no dispara WhatsApp por su
   cuenta. La torta se deposita en el mismo pedido que usa el catalogo
   (SENTIDA.pedido) y de ahi sale una sola solicitud. Antes habia dos
   flujos que no se hablaban: se podia mandar la torta por un lado y
   los alfajores por otro.
   ============================================================ */
(function () {
  'use strict';
  ...
  var CLAVE = 'sentida-v3-torta';
  var S = window.SENTIDA;
  ...
  function pintarResumen() {
    ...
    // WhatsApp queda como salida de emergencia, no como el final del
    // camino: el final es "Agregar a mi pedido".
    // El enlace se arma siempre, no sólo después de tocar algo:
    ...
    if (wa) {
      ...
      wa.href = 'https://wa.me/5491158300787?text=' +
        encodeURIComponent('Hola SENTIDA! Estoy armando una torta decorada:\n\n' + cuerpo);
    }
  }
  ...
  // Paso final: la torta entra al pedido y la persona sigue en un solo
  // lugar, junto con lo que haya elegido del catalogo.
  var sumar = $('sumar-torta');
  if (sumar) {
    sumar.addEventListener('click', function () {
      var f = filas();
      if (!f.length) return;
      S.pedido.guardarTorta(f.map(function (x) { return [x[0], x[1]]; }), '');
      try { localStorage.removeItem(CLAVE); } catch (e) {}
      location.href = 'pedido.html?sumada=torta';
    });
  }
